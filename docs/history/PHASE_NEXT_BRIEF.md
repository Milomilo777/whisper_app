...
### Build batch + post-build verification (`build.bat`)

A teammate's field note: **PyInstaller builds for Windows commonly fail at runtime because the bundled binaries (`bin/ffmpeg.exe`, `bin/ffprobe.exe`, `bin/yt-dlp.exe`) and runtime config are not present in `dist/`.** The spec file's `datas=[('bin', 'bin')]` is supposed to handle `bin/`, but: (a) the file count is large and PyInstaller sometimes silently drops a binary if a hook misfires, (b) the result is a build that compiles cleanly and crashes the moment the user opens the Download Videos tab.

The defense: a committed `build.bat` that calls PyInstaller and then **verifies** the critical files are present, with a non-zero exit on mismatch. Author this file at the repo root.
...
- Commit `build.bat` at the repo root
- Run `build.bat clean` for the production build step
- Run `build.bat verify` if the binary already exists (idempotent re-check)
- Use `build.bat smoke` as the **canonical compile-test** instead of the inline Python smoke in the next section — but keep the Python smoke as an additional cross-check
- Document in `docs/BUILD.md`: when to use which mode, what each exit code means, and why config.json is **not** in dist (Phase 1.2 migration)
