"""Model download dialog — drives :func:`core.model_manager.ensure_model`.

Modal Toplevel with a determinate progress bar + speed + ETA.
Cancel-aware via :class:`threading.Event`.
"""
...
class ModelDownloadDialog(tk.Toplevel):
    """Modal "Preparing Whisper model" dialog.

    Attributes
    ----------
    success: bool
        True only when ``ensure_model`` returned normally.
    error: str | None
        Set to a stringified exception if the download failed for a
    ...
    def __init__(self, master: "tk.Tk | tk.Toplevel") -> None:
        ...
        ttk.Label(
            body, text="Downloading required model",
        ...
        ttk.Label(body, textvariable=self.detail_var).grid(
            row=3, column=0, columnspan=2, sticky="w",
        )
        ttk.Label(body, textvariable=self.elapsed_var).grid(
            row=4, column=0, sticky="w", pady=(10, 0),
        )
        ttk.Label(body, textvariable=self.remaining_var).grid(
        ...
        threading.Thread(target=self._worker, name="model-download-dialog",
                         daemon=True).start()
    ...
    def _poll(self) -> None:
        ...
        self.elapsed_var.set(f"Elapsed: {_fmt_duration(elapsed)}")
        self.after(100, self._poll)
    ...
    def _apply_progress(self, payload: dict[str, Any]) -> None:
        ...
        if total:
            self.size_var.set(f"Total: {_fmt_bytes(total)}")
        if downloaded is not None and total:
            self.size_var.set(
                f"Total: {_fmt_bytes(downloaded)} / {_fmt_bytes(total)}"
            )
        if speed:
            self.speed_var.set(f"Speed: {_fmt_bytes(speed)}/s")
        if "remaining" in payload:
            self.remaining_var.set(
                f"Remaining: {_fmt_duration(payload.get('remaining'))}"
            )
