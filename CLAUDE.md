...
the v1.0.1 release. The following are pre-authorised for all future
hands-off sessions:
...
asks for them in the current session:
  ...
  - Editing `.git/config`
  - `git push --force` against anything (v1.0.1 is already public —
...
## Style & scope
  ...
  - Pyright must report 0 errors on `app/` and `core/` before
    every commit. The v1.0.1 baseline is 0 errors / 0 warnings /
