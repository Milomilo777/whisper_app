...
def test_srt_roundtrip_persian():
    ...
    for (o_start, _o_end, o_body), (r_start, _r_end, r_body) in zip(
        ...
        assert abs(o_start - r_start) < 0.001
        assert o_body == r_body
    # The .otr text must keep Persian characters verbatim.
    otr_string = srt_to_otr(str(FIXTURES / "sample_persian.srt"), "audio.wav")
    assert "سلام" in json.loads(otr_string)["text"]
