...
REM installer_embed.iss turns into WhisperProjectBasic-vX.Y.Z-Setup.exe.
REM
...
REM   core\                     transcription / paths modules
REM   bin\                      ffmpeg.exe, ffprobe.exe (no yt-dlp)
...
"%BUILD%\python\python.exe" -c "import faster_whisper, ctranslate2, sv_ttk, platformdirs, tkinter, tkinterdnd2; print('embed_import_ok')"
if errorlevel 1 (
  echo [embed] sanity import failed - bundle is incomplete
  exit /b 5
)

echo [embed] verifying gui.py entry point parses
